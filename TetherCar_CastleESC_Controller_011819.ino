//This software is intended to be used in an electric teather car with an electrical safety shutoff switch.
//
//This software is intended to be used with a Castle Phoenix Edge HV 120 ESC programmed for no arming sequence.
//According to Thomas form Castle, the high/low PWM is 1.3/1.8 milli seconds respectively with with a resolution of 256 steps in between.
//1800 - 1300 micro seconds = 500 with 256 steps, the increments are need to be a minimum of 1.95.
//However, for ease of programming, we just increment by 1 microsecond from 1300 to 1800 in the ramp up duration, thus making every 2nd increment actually be recognized by the ESC.
//This must be programmed into the ESC with their field programmer or computer based programmer.
//
//Matt Geders 01/18/19.  mgeders@gmail.com
...
int LowIdle = 1300;		                            // The lowest "throttle" (PWM) for the ESC. (should produce zero RPM)
...
int RampUpDuration = 15;                          // The number of seconds the ramp up will occur.  For reasons of Arduino limitations, recomment don't go lower than 7 seconds.
...
void setup()                                      // This only runs once upon boot up of the Arduino itself.  Think if it as the "setup" commands.  We only use this because we have a 1 time run of the program.
{
  ...
  digitalWrite(RXLED, HIGH);                      // set the RX LED OFF                          
    ...
    do						                                // This loop is to send a LowIdle signal so the ESC recognizes its connected and arms, but doesn't actually move. It is intent is to allow it to retry sending the minimum signal in case it finishes before the ESC boots
    {
      ...
      delay(1000);				                        // wait a second (milli-seconds)
...
if (buttonState == LOW){goto Shutdown;}           // If the switch is flicked back (open) then goto directly to the "Shutdown" portion of the code
    ...
    for(i = 0; i <(MaxSpeed-MinSpeed); i++)       // This sequence ramps the motor to max speed over a defined duration. "i" is just counter that starts at 0 and goes to MaxSpeed-Minspeed)  "i++" just increases "i" by one each time this loops
    {
	    esc.writeMicroseconds(MinSpeed++);          // Takes the MinSpeed and adds 1 to it and stores that new value as the new MinSpeed
      buttonState = digitalRead(switchPin);       // Checks the state of the SPST safety switch
      if (buttonState == LOW){goto Shutdown;}     // If the switch is flicked back (open) then goto directly to the "Shutdown" portion of the code
      digitalWrite(RXLED, LOW);                   // set the RX LED ON
      delay(round(((RampUpDuration*1000)/(MaxSpeed-MinSpeed))));  // Self explanatory.  The low light yellow blink speed will effectively vary based on this line.
      digitalWrite(RXLED, HIGH);                  // set the RX LED OFF
    }
...
Shutdown:					                                // This is the Shutdown label of the code.  If the SPST safety switch is tripped forward, the program exits and goes here.
  TXLED0;                                         // TX LED is not tied to a normally controlled pin so a macro is needed, turn LED OFF
  digitalWrite(RXLED, HIGH);                      // set the RX LED OFF  
